<template>
  <div class="speech_container">
    <i
      class="fa fa-microphone fa-3x" :class="{ isListening }" @click="listen"></i>
...
export default {
    ...
    methods :{
        ...
        record(event){
            ...
            const removeAllNotesRegex=/tüm notları sil/giu;
            const removeAllNotesMatch = removeAllNotesRegex.test(this.speechToText);
            console.log(voiceMatch);
            console.log(removeAllNotesMatch);
            setTimeout(()=>{
                if(voiceMatch && voiceMatch.groups.id && voiceMatch.groups.command)
                {
                    this.deleteNote(voiceMatch.groups.id);
                }
                else if(removeAllNotesMatch)
                {
                    this.removeAllNotes();
                }
                else
